...
def _mock_intent(intent_type: IntentType = IntentType.METRIC, requires_comparison: bool = False) -> Intent:
    """Create a mock intent for testing (A8 schema).
    
    Args:
        intent_type: Type of intent to mock
        requires_comparison: Whether query requires comparison
    
    Returns:
        Mock Intent object with A8 schema
    """
    return Intent(
        type=intent_type,
        confidence=0.95,
        rationale=f"Mock {intent_type.value} intent for testing",
...
def test_planner_top_5_barbers_grouped_with_row_limit():
    ...
    with patch("haikugraph.planning.llm_planner.classify_intent", return_value=_mock_intent()), \
         patch("haikugraph.planning.llm_planner.call_llm") as mock_llm:
        mock_llm.return_value = _fake_llm_return(valid_plan)
